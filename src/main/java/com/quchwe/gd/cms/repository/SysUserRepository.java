...
public interface SysUserRepository extends JpaRepository<SysUser, Long> {
    ...
    @Query("UPDATE SysUser u SET u.address =?1,u.age =?2,u.carType = ?3,u.drivingLicenseId =?4,u.email=?5,u.headImage =?6," +
            "u.purchaseDate=?7,u.sex=?8,u.signature=?9,u.updateTime=?10 ,u.userName=?11 WHERE u.phoneNumber = ?12")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE SysUser u SET u.userToken =?1 WHERE u.phoneNumber = ?2")
    int updateUserToken(String userToken, String phoneNumber);
